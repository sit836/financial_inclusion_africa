...
# submission
enc = OneHotEncoder(handle_unknown='ignore')
X_processed_train = pd.DataFrame(np.hstack((X_train_raw[NUM_FEAS], enc.fit_transform(X_train_raw[CAT_FEAS]).todense())),
                                 columns=NUM_FEAS + list(enc.get_feature_names_out()))
...
print(df_train['uniqueid'] + ' x ' + df_train['country'])
quit()
